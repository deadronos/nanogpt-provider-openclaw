...
/**
 * Registers the provider manually and returns the wrapStreamFn.
 */
 ...
 */
async function testToolEnabledRequest(wrapStreamFn: any) {
 ...
 */
async function testNonToolRequest(wrapStreamFn: any) {
  let capturedPayload: any = null;
  const capturedPayloadRef = { current: null };
  const baseStreamFn = createMockStreamFn(capturedPayloadRef);
 ...
 */
async function testExistingResponseFormat(wrapStreamFn: any) {
  let capturedPayload: any = null;
  const capturedPayloadRef = { current: null };
...
async function main() {
  const wrapStreamFn = setupProvider();

  await testToolEnabledRequest(wrapStreamFn);
  await testNonToolRequest(wrapStreamFn);
  await testExistingResponseFormat(wrapStreamFn);
}
