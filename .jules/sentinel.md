...
## 2026-06-18 - [Prevent Over-Redaction of Safe Metrics in Logger]
**Vulnerability:** When fixing the logging of compound sensitive keys (like `providerApiKey`), using a simple regex (`/apikey|token/i`) causes legitimate operational metrics (like `prompt_tokens`, `completion_tokens`, `total_tokens`) to be falsely flagged as sensitive and redacted, breaking observability.
**Learning:** Security fixes must consider operational impact. A naive substring match for "token" will redact standard LLM token usage metrics, obscuring valuable telemetry data.
**Prevention:** Always pair generalized substring redaction patterns with an explicit exclusion list (`SAFE_METRICS`) for known-safe keys, ensuring sensitive data is caught without destroying operational logs.
